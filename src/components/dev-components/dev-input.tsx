...
const DevInput = forwardRef<HTMLInputElement, InputProps>(
  (
    ...
    const commonStyle = `w-full flex border border-accentGray/50 transition-all ring-accentBlue/50 items-center ${
      icon && " gap-2 "
    },
    ...
    const inputVariants = {
      ...
      underline:
        "border-0 !ring-0 !border-accentBlue/50 border-b-4 relative after:content-[''] after:absolute after:h-1 after:bg-accentBlue after:-bottom-1 after:w-full after:scale-x-0  after:transition after:duration-300 after:origin-center rounded-none px-0 has-[:focus]:after:scale-x-100",
    };
    ...
    return (
      <div>
        ...
        >
          <span className="text-xl text-accentBlue">{icon}</span>
          <input
            ref={ref}
