...
const SidebarChatList = ({ sidebarList }: any) => {
  ...
  const { updateChatInSidebar, removeChatFromSidebar, refreshSidebar } = useSidebarContext();
  ...
  const handleDeleteChat = async (chatID: string) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Remove chat from sidebar immediately
        removeChatFromSidebar(chatID);
        // Navigate to main app page
        router.push("/app");
      } else {
        setToast(result.error || "Failed to delete chat");
      }
    } catch (error) {
  ...
  const handleRenameChat = async (
    ...
    console.log('Frontend handleRenameChat called with:', { chatID, chatInfo });
    setModalLoader(true);
    try {
      const result = await renameChat(chatID, chatInfo);
      console.log('Frontend rename result:', result);
      if (result.success) {
        setModalOpen(false);
        setToast("Chat renamed successfully");
        // Update sidebar immediately without full refresh
        updateChatInSidebar(chatID, { title: chatInfo.title, icon: chatInfo.icon });
      } else {
        setToast(result.error || "Failed to rename chat");
      }
    } catch (error) {
      console.error("Failed to rename chat:", error);
      setToast("An error occurred while renaming");
    } finally {
      setModalLoader(false);
    }
  };
        ...
        return (
          <div className="mt-5">
            <p>
              ...
              activity like prompts, responses, and feedback from your AI assistant
              Apps Activity.
            </p>
              ...
              >
                ...
                >
                  ...
                  <p className="truncate text-left">
                    {item?.chatInfo?.title || item?.message?.userPrompt || "New Chat"}
                  </p>
                </Link>
                {item?.isPinned && (
                  ...
                  <div className="overflow-hidden py-2 min-w-44">
                    <DevButton
                      onClick={async () => {
                        try {
                          setTopLoader(true);
                          const result = await pinChat(item.chatID, !item.isPinned);
                          if (result.success) {
                            if (item.isPinned) {
                            ...
                            // Refresh sidebar data to reflect pin changes (which affect ordering)
                            refreshSidebar();
                          } else {
                            setToast(result.error || "Failed to update pin status");
                          }
                        } catch (error) {
